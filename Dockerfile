FROM --platform=${BUILDPLATFORM} ubuntu:jammy-20240227
...
RUN if [ "${SLIM}" = "no" ]; then \
    if [ "${TARGETPLATFORM}" = "linux/amd64" ]; then ARCHITECTURE=amd64; elif [ "${TARGETPLATFORM}" = "linux/arm64" ]; then ARCHITECTURE=arm64; else ARCHITECTURE=amd64; fi ;\
    DOWNLOAD_URL="$( curl -LsS https://api.github.com/repos/tofuutils/tenv/releases/latest | grep -o -E "https://.+?_${ARCHITECTURE}.deb" | head -1)" ;\
    echo -e "Downloading tenv from ${DOWNLOAD_URL}" ;\
    for i in {1..5}; do curl -LsS "${DOWNLOAD_URL}" -o ./tenv.deb && break || sleep 15; done ;\
    dpkg -i ./tenv.deb ;\
    rm -f ./tenv.deb ;\
  fi
